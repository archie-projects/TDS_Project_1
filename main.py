...
# Uncomment if you use dotenv file to load env variables
...
# CORS configuration
# When using allow_origins=["*"], allow_credentials must be False.
# Otherwise, specify allowed origins explicitly.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # For production, specify your frontend URL(s) here
...
discourse_posts: List[Dict] = []
aiproxy_token: Optional[str] = None
embeddings_cache: Dict[str, List[List[float]]] = {}
...
QUESTION_PATTERNS = {
    'gpt_model': {
        ...
        'response': (
            ...
            "Use the OpenAI API directly for assignments that specify this model."
        )
    },
    'installation': {
        'keywords': ['install', 'setup', 'pip', 'conda', 'error', 'import'],
        'response': (
            "For installation issues, refer to the Development Tools section. "
            "Use `uv` for Python package management as recommended in the course."
        )
    },
    'assignment': {
        'keywords': ['assignment', 'homework', 'submission', 'deadline', 'grade'],
        'response': (
            "For assignment-related questions, check the course timeline and submission guidelines. "
            "Make sure to follow the exact specifications provided."
        )
    },
    'discourse': {
        'keywords': ['discourse', 'forum', 'post', 'discussion'],
        'response': (
            "You can find discussions and additional help on the course discourse forum. "
            "Check existing posts before creating new ones."
        )
    }
}

def load_data():
    """Load course content and discourse data from JSONL files and load token from env."""
    ...
    try:
        with open('CourseContentData.jsonl', 'r', encoding='utf-8') as f:
            course_content = [json.loads(line.strip()) for line in f if line.strip()]
    ...
    try:
        with open('DiscourseData.jsonl', 'r', encoding='utf-8') as f:
            discourse_posts = [json.loads(line.strip()) for line in f if line.strip()]
        print(f"Loaded {len(discourse_posts)} discourse posts")
    except FileNotFoundError:
        print("DiscourseData.jsonl not found")

def get_embeddings(texts: List[str]) -> List[List[float]]:
    """Get embeddings for texts using AI Proxy, with caching."""
    if not aiproxy_token or not texts:
        return []
    ...
    try:
        response = requests.post(f"{AIPROXY_BASE_URL}/v1/embeddings", headers=headers, json=payload, timeout=30)
...
def find_relevant_content(question: str, top_k: int = 5) -> List[Dict]:
    """Find relevant course and discourse content using semantic search."""
    if not aiproxy_token:
        return []
    ...
    for post in discourse_posts:
        all_content.append({
            ...
            'url': post.get('url', ''),
            'type': 'discourse',
            'source': f"@{post.get('username', 'Unknown')}",
...
def match_question_pattern(question: str) -> Optional[str]:
    """Return canned response if question matches known pattern."""
...
def generate_answer_with_aiproxy(question: str, relevant_content: List[Dict], image_description: Optional[str] = None) -> str:
    """Generate an answer using AI Proxy chat completion."""
    if not aiproxy_token:
        return generate_fallback_answer(question, relevant_content)

    context = ""
    for i, content in enumerate(relevant_content[:3]):
        snippet = content['text'][:500].replace('\n', ' ').strip()
    ...
    user_prompt = f"Question: {question}\n\nContext from course materials and discussions:{context}"
    ...
    try:
        response = requests.post(f"{AIPROXY_BASE_URL}/v1/chat/completions", headers=headers, json=payload, timeout=30)
        response.raise_for_status()
        data = response.json()
...
def generate_fallback_answer(question: str, relevant_content: List[Dict]) -> str:
    """Simple fallback answer based on most relevant content or default message."""
    if relevant_content:
        top_content = relevant_content[0]
        snippet = top_content['text'][:300].replace('\n', ' ').strip()
        return f"Based on the course materials and discussions: {snippet}..."
    else:
...
def generate_answer(question: str, relevant_content: List[Dict]) -> str:
    """Generate answer combining pattern matching and semantic search with AI Proxy."""
...
def extract_links(relevant_content: List[Dict]) -> List[LinkResponse]:
    """Extract top relevant links for response."""
    links = []
    for content in relevant_content[:3]:
        ...
        links.append(LinkResponse(url=content['url'], text=link_text))
    return links
