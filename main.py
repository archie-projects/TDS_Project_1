...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "http://127.0.0.1:3000",
        "http://127.0.0.1:8080",
        "*"  # Keep this as fallback, but specific origins are preferred
    ],
...
async def options_api():
    """Handle preflight OPTIONS request for /api/ endpoint"""
    return {"message": "OK"}

@app.options("/")
async def options_root():
    """Handle preflight OPTIONS request for / endpoint"""
    return {"message": "OK"}
...
# Also add POST handler for root endpoint since the frontend is trying to access it
